...
impl LineSeparator {
    ...
    pub fn print<T: Write+?Sized>(&self, out: &mut T, col_width: &[usize], colsep: bool, lborder: bool, rborder: bool) -> Result<(), Error> {
        if lborder {
            try!(out.write_all(self.ljunc.encode_utf8().as_slice()));
        }
        let mut iter = col_width.into_iter().peekable();
        while let Some(width) = iter.next() {
            for _ in 0..*width+2{
                try!(out.write_all(self.line.encode_utf8().as_slice()));
            }
            if colsep && iter.peek().is_some() {
                try!(out.write_all(self.junc.encode_utf8().as_slice()));
            }
        }
        if rborder {
            try!(out.write_all(self.rjunc.encode_utf8().as_slice()));
        }
